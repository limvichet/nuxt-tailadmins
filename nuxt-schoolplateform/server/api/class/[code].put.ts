...
export default defineEventHandler(async (event) => {
  ...
  const body: ClassUpdateREQ = await readBody(event);
  ...
  try {
    ...
    const parsedBody = classUpdateREQ.parse(body);

    // send request to backend
    const res = await $fetch<ClassUpdateRES>(
    ...
    throw customCreateError(error, `Can't update class ${code}!`);
  }
});
