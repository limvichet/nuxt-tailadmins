...
export default defineEventHandler(async (event) => {
  ...
  const body: StaffUpdateREQ = await readBody(event);
  ...
  try {
    ...
    const parsedBody = staffUpdateREQ.parse(body);

    // send request to backend
    const res = await $fetch<StaffUpdateRES>(
    ...
    throw customCreateError(error, `Can't update staff ${code}!`);
  }
});
