...
import { useUpdateForm } from "~/composables/school/staff-teaching/update";
...
use1EZSeo({
  ...
  description: `This is the editing page for staff teaching with code ${code}.`,
});

const { update, cancelForm } = useUpdateForm(code);
const {
  meta,
  errors,
  fields,
...
if (asyncData.error.value) {
  throw createError({
    ...
    statusMessage: "Can't update staff teaching without code!",
    fatal: true,
  });
}
</script>
        ...
        >
          ...
          <div class="grid gap-2 required-field">
            ...
            <CustomComboboxWithList
              id="staff_id"
              button-class="w-full"
              :options="defaultValues?.options?.staffs ?? []"
          ...
          <div class="grid gap-2 required-field">
            ...
            <CustomComboboxWithList
              id="cgt_id"
              button-class="w-full"
              :options="defaultValues?.options?.grade_types ?? []"
